...
def get_db_engine():
    ...
    try:
        ...
        if not all([DB_HOST, DB_DATABASE, DB_USER, DB_PASSWORD]):
            raise ValueError("One or more database environment variables are missing (DB_HOST, DB_DATABASE, DB_USER, DB_PASSWORD).")
        ...
        st.error(f"Error connecting to database. Please check your environment variables and network access: {e}")
        st.stop() 
...
def check_credentials():
    """Authenticates user against credentials from environment variables."""
    try:
        ...
        if not all([CORRECT_USERNAME, CORRECT_PASSWORD]):
            raise ValueError("App authentication environment variables are missing (APP_USERNAME, APP_PASSWORD).")

    except ValueError as e:
        st.error(f"Authentication setup error: {e}. Please ensure APP_USERNAME and APP_PASSWORD are set in your environment variables.")
    ...
    if st.session_state.get('username') == CORRECT_USERNAME and st.session_state.get('password') == CORRECT_PASSWORD:
...
if not st.session_state['authenticated']:
    ...
    st.text_input("Username", key="username", on_change=check_credentials)
    st.text_input("Password", type="password", key="password", on_change=check_credentials)
    st.stop()
else:
    # --- Main App Content (visible after authentication) ---
    # Retrieve username from environment for display
    displayed_username = os.environ.get("APP_USERNAME", "User")
    st.sidebar.success(f"Logged in as '{displayed_username}'")
        ...
        if selected_reference_contract_symbol:
            ...
            if not match:
                ...
                for i in range(num_years_to_compare):
                    ...
                    df_contract_history = get_contract_data_from_db(table_name)
